...
{
    private $supportTablesChecked = null;
    ...
    // }
     ...
     public function supportTicketUpdate()
    {
        header('Content-Type: application/json');
        ...
        try {
            ...
            if (!$ticket_id || !$action) {
                throw new Exception('Ticket ID and action are required');
            }

            // Get current ticket status
            $current_status = $this->getTicketStatus($ticket_id);
            if (!$current_status) {
            ...
            // Additional validation for reopen action
            if ($action === 'reopen' && !in_array($current_status, ['closed', 'resolved'])) {
                throw new Exception('Only closed or resolved tickets can be reopened');
            }

            $success = $this->updateTicketStatus($ticket_id, $new_status);
        ...
        try {
            ...
            if ($ticket_id) {
                ...
                if ($ticket_status !== 'open') {
                    throw new Exception('Cannot send message to closed or resolved ticket');
                }
    ...
    private function checkSupportTables()
    {
        ...
        try {
            // Check both tables separately with proper error handling
